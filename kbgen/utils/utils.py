...
def replace_readout(m, name, hist=''):
    from mup import MuReadout

    for attr_str in dir(m):
        target_attr = getattr(m, attr_str)
        if type(target_attr) == torch.nn.Linear and attr_str == "readout":
            print("mup replaced: ", hist, name, attr_str)
            fan_out, fan_in = target_attr.weight.shape
            setattr(m, attr_str, MuReadout(fan_in, fan_out))
    for n, ch in m.named_children():
        replace_readout(ch, n, '.'.join([hist, name]))
...
def mup_model(
    ...
    replace_readout(model, "model")
    # set up the correct init
    model = set_base_shapes(model, base_model, delta=delta_model)
    for name, param in model.named_parameters():
        if "readout" in name:
            param.data.zero_()
        elif "weight" in name:
            init.normal_(param, mean=0.0, std=config["init_var"])
        elif "bias" in name:
            param.data.zero_()
        else:
